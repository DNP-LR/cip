...
interface EditTaskModalProps {
    ...
    onUpdateTask: (taskId: string, updates: Partial<Omit<Task, 'id'>>) => Promise<Task>;
}

const EditTaskModal: React.FC<EditTaskModalProps> = ({isOpen, onClose, task, onTaskUpdated, onUpdateTask}) => {
    ...
    const handleSubtaskEdit = (index: number, newText: string) => {
        const updatedSubtasks = [...subtasks];
        updatedSubtasks[index] = { ...updatedSubtasks[index], text: newText };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setError("Failed to update task. Please check the console for details.");
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden border border-[#A09BAD]/30">
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4 max-h-[80vh] overflow-y-auto">
                            ...
                            {subtasks.map((subtask, index) => (
                                <li key={index}
                                    ...
                                    <input
                                        ...
                                        className="text-sm text-[#131427] flex-grow bg-transparent outline-none"
                                    />
                                    <button
                                        type="button"
                    ...
                    <div className="flex justify-end gap-3 pt-4 border-t border-gray-100">
                        ...
                        >
                            {loading ? 'Modification...' : 'Mettre à Jour la Tâche'}
                        </button>
                    </div>
                </form>
            </div>
